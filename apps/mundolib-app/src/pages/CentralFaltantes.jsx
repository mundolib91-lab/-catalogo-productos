...
function CentralFaltantes({ menuHamburguesa }) {
  ...
  const cargarFaltantes = async () => {
    ...
    try {
      ...
      params.append('tienda', APP_CONFIG.tienda);
      ...
      const paramsContadores = new URLSearchParams();
      paramsContadores.append('tienda', APP_CONFIG.tienda);
      paramsContadores.append('limit', '1000');

      const responseContadores = await fetch(
        `${import.meta.env.VITE_API_URL}/faltantes?${paramsContadores.toString()}`
      );
